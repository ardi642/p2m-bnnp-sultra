...
return new class extends Migration
{
    ...
    {
        Schema::create('pegawai_p2m_lingkungan', function (Blueprint $table) {
            $table->id();
            
            // 1. Relasi ke Kegiatan (Masih pakai ID/Integer standar)
            $table->foreignId('p2m_lingkungan_id')
                ->constrained('p2m_lingkungan')
                ->onDelete('cascade');
            ...
            $table->unique(['p2m_lingkungan_id', 'pegawai_nip'], 'unique_kegiatan_pegawai');
        });
    }
    ...
    {
        Schema::dropIfExists('pegawai_p2m_lingkungan');
    }
};
