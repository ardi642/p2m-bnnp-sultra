            ...
            <div class="col-12">
                <div class="card border-0 shadow-sm bg-white rounded-4">
                    <div class="card-header bg-transparent border-0 pt-4 pb-2 d-flex flex-wrap justify-content-between align-items-center gap-2">
                        <template x-if="isMultiSatker">
                            <div class="d-flex align-items-center bg-light rounded-pill px-3 py-1">
                                ...
                                <select x-model="demografi.compView" class="form-select form-select-sm border-0 bg-transparent text-dark fw-bold shadow-none cursor-pointer pe-4">
                    ...
                    <div class="card-body px-4 pb-4 pt-4">
                        ...
                        {{-- APEXCHART GLOBAL (FULL WIDTH / HEATMAP) --}}
                        ...
                        {{-- HORIZONTAL BAR MINI UNTUK SEMUA PANEL (Karena Mode Selalu Snapshot) --}}
        ...
        <div class="bg-white p-4 rounded-4 shadow-sm mb-3 border">
            <div class="d-flex flex-column flex-xl-row justify-content-between align-items-xl-center gap-3">
                ...
                <div class="d-flex flex-wrap gap-3 align-items-center">
                    ...
                    {{-- KEMBALIKAN FILTER WAKTU UNTUK RANKING --}}
                    <div class="d-flex align-items-center bg-light rounded-3 px-3 py-1">
                        <i class="bi bi-filter text-muted me-2"></i>
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('dashboardRehab', () => ({
            ...
            chartInst: { layanan: null, demografi: null, rank: null, panelDemo: [] },
            
            getBarColors() { return ['#0d6efd', '#ffc107', '#198754', '#0dcaf0', '#6f42c1', '#dc3545', '#20c997', '#fd7e14', '#6c757d', '#e83e8c', '#adb5bd', '#212529']; },
            getLayananColors() { return ['#0d6efd', '#ffc107', '#198754']; }, // RJ, Pasca, SKHPN

            // FUNGSI WARNA CERDAS (Bebas warna kembar)
            getCategoryColor(catName, index = 0) {
                ...
                // Ambil warna berurutan dari palet berdasarkan indexnya
                let palette = this.getBarColors();
                return palette[index % palette.length];
            },

            resolveColors(series, isHeat, tabName) {
                if (isHeat) return ['#dc3545'];
                
                // Jika cuma 1 Satker, warna ada di dalam bar/kategori
                if (!this.isMultiSatker) {
                    return this.getBarColors();
                }

                // Jika Multi Satker, beri warna berbeda untuk setiap Series (Kategori)
            ...
            showEmptyState(refElement) {
                if(refElement) refElement.innerHTML = '<div class="d-flex flex-column align-items-center justify-content-center text-muted" style="min-height: 350px;"><i class="bi bi-inbox fs-1 mb-2"></i><h6 class="fw-bold">Tidak Ada Data</h6><p class="small">Belum ada inputan untuk filter yang dipilih.</p></div>';
            },

            layanan: { time: 'all', tabTrend: 'rj', adminTrendType: 'bar', data: null },
            demografi: { time: 'all', tabComp: 'sumber', compView: 'panel', modeHitung: 'layanan', data: null },
            ...
            init() {
                ...
                // Pisahkan watcher rank dan demografi agar independen (hanya berbagi modeHitung)
                this.$watch('demografi.modeHitung', () => {
                    this.fetchDemografi();
                    this.fetchRank(); // Update juga Ranking jika mode hitung berubah
                });
                
                ['time', 'compView'].forEach(p => this.$watch('demografi.'+p, () => this.fetchDemografi()));
            ...
            // VISIBILITY ROUTER
            showGlobalChart() {
                let st = this.demografi;
                if (!this.isMultiSatker) return true; 
                if (st.compView === 'heatmap') return true;
                return !this.showMiniBarPanel();
            },
            showMiniBarPanel() {
                return (this.isMultiSatker && this.demografi.compView === 'panel');
            },
            ...
            fetchAll() { this.fetchGlobal(); this.fetchLayanan(); this.fetchDemografi(); this.fetchRank(); },
            ...
            fetchGlobal() { 
                ...
                .then(r=>r.json()).then(res => { 
                    this.cards = res; 
                    // Kalkulasi persentase KPI di Frontend
            ...
            renderLayanan() {
                ...
                if (this.layanan.tabTrend === 'gabungan') {
                    ...
                    // Pewarnaan dinamis untuk mode single metric
            ...
            renderDemografi() {
                ...
                // 1. RENDER HORIZONTAL BAR MINI (Semua Kategori jadi Horizontal Bar)
                        ...
                        pDataArr.forEach((pData, pIdx) => {
                            ...
                            if (el && pData.items && pData.items.length > 0) {
                                ...
                                let opts = {
                                    ...
                                    colors: categories.map((cat, i) => this.getCategoryColor(cat, i)), // Warna Berurutan Aman
                ...
                // 2. RENDER CHART GLOBAL (FULL WIDTH)
                const names = {'sumber':'Sumber Pasien', 'gender':'Gender', 'usia':'Kelompok Usia', 'pendidikan': 'Pendidikan', 'pekerjaan':'Pekerjaan'};
                const isHeat = false; // Disable heatmap for Demografi as per current flow logic or you can enable it
                ...
                let opts = {
                    ...
                    colors: this.resolveColors(dComp.series, false, this.demografi.tabComp),
