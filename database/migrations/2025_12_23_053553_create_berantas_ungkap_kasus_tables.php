...
return new class extends Migration
{
    public function up(): void
    {
        ...
        Schema::create('berantas_ungkap_tersangka', function (Blueprint $table) {
            $table->id();
            $table->foreignId('berantas_ungkap_kasus_id')->constrained('berantas_ungkap_kasus')->cascadeOnDelete();
            
            $table->string('nama_tersangka');
            $table->enum('jenis_kelamin', ['Laki-Laki', 'Perempuan']);
            $table->string('pekerjaan')->nullable();
            $table->string('tahap'); // Wajib diisi, tanpa default
        ...
        // 4. TABEL PIVOT (Milik Bersama / Many-to-Many)
        // Tidak perlu Model khusus untuk tabel ini
