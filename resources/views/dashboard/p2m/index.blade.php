            ...
            <div class="col-md-6">
                <div class="card border-0 shadow-sm h-100 bg-white border-start border-4 border-success rounded-3 overflow-hidden">
                    <div class="card-header bg-white border-0 pt-4 pb-2 d-flex align-items-center gap-3">
                        ...
                        <div>
                            <h6 class="text-muted fw-bold mb-0">Masyarakat Terlayani</h6>
                            <h3 class="fw-bold text-dark mb-0">
                                <span x-text="formatAngka(cards.orang.total)">0</span> 
                                <span class="fs-6 text-muted fw-normal">Orang</span>
                            </h3>
                        </div>
                    </div>
                    <div class="card-body pt-0 overflow-auto" style="max-height: 220px;">
                        <template x-for="(item, label) in cards.orang.list">
                            <div x-show="(item.val || item) > 0" class="d-flex justify-content-between align-items-center border-bottom pb-2 mb-2">
                                ...
                                <div class="text-end">
                                    <span class="fw-bold text-success" x-text="formatAngka(item.val || item)"></span>
                                    <template x-if="item.is_tes_urine">
                                        <div class="lh-1 mt-1">
                                            <span class="badge bg-danger-subtle text-danger border border-danger">Positif: <span x-text="item.positif"></span></span>
                                        </div>
                                    </template>
                                </div>
                            </div>
                ...
                <div class="card border-0 shadow-sm h-100 bg-white border-start border-4 border-warning rounded-3 overflow-hidden">
                    <div class="card-header bg-white border-0 pt-4 pb-2 d-flex align-items-center gap-3">
                        ...
                        <div>
                            ...
                            <h3 class="fw-bold text-dark mb-0">
                                <span x-text="formatAngka(cards.media.total_freq)">0</span> 
                                <span class="fs-6 text-muted fw-normal">Publikasi</span>
                            </h3>
                        </div>
                    </div>
                ...
                <div class="card border-0 shadow-sm h-100 bg-white border-start border-4 border-info rounded-3 overflow-hidden">
                    <div class="card-header bg-white border-0 pt-4 pb-2 d-flex align-items-center gap-3">
                        ...
                        <div>
                            ...
                            <h3 class="fw-bold text-dark mb-0">
                                <span x-text="formatAngka(cards.wilayah.total)">0</span> 
                                <span class="fs-6 text-muted fw-normal">Kawasan</span>
                            </h3>
                        </div>
                    </div>
                    <div class="card-body pt-0 d-flex flex-column justify-content-center">
                ...
                <div class="d-flex flex-wrap gap-3 align-items-center">
                    ...
                    <div class="d-flex align-items-center bg-light rounded-3 px-3 py-1">
                        ...
                        <select x-model="detailType" 
                                class="form-select border-0 bg-transparent fw-bold text-dark shadow-none cursor-pointer pe-4" 
                    ...
                    <div class="d-flex align-items-center bg-light rounded-3 px-3 py-1">
                        ...
                        <select x-model="detailMode" 
                                class="form-select border-0 bg-transparent fw-bold text-dark shadow-none w-auto cursor-pointer pe-4">
                        ...
                        <template x-if="detailMode === 'monthly'">
                            <select x-model="detailMonthYear" 
                                    class="form-select border-0 bg-transparent fw-bold text-dark shadow-none w-auto ms-1 cursor-pointer pe-4">
                                @foreach($years as $y) <option value="{{ $y }}">{{ $y }}</option> @endforeach
                            </select>
                        </template>
                        ...
                        <template x-if="detailMode === 'yearly'">
                            <div class="d-flex align-items-center ms-1 bg-white rounded">
                                <select x-model="detailYearStart" 
                                        class="form-select form-select-sm border-0 bg-transparent fw-bold text-dark shadow-none w-auto cursor-pointer p-1 pe-3">
                                    @foreach($years as $y) <option value="{{ $y }}">{{ $y }}</option> @endforeach
                                </select>
    ...
    document.addEventListener('alpine:init', () => {
        Alpine.data('dashboardP2M', () => ({
            ...
            get dynamicTrendMetric() { 
                return this.tabTrend === 'kegiatan' ? 'Jumlah Kegiatan' : (this.tabTrend === 'peserta' ? 'Jumlah ' + this.config.unit : 'Indikasi Positif'); 
            },
            ...
            get dynamicTrendTitle() { 
                return this.detailMode === 'monthly' ? '(Januari - Desember ' + this.detailMonthYear + ')' : '(' + this.detailYearStart + ' s/d ' + this.detailYearEnd + ')'; 
            },
            
            get dynamicCompTitle() {
                if (this.detailMode === 'monthly') {
                    if (this.detailMonthMonth === 'all') return '(Akumulasi Tahun ' + this.detailMonthYear + ')';
            ...
            renderTrend() {
                ...
                if (isHeatmap) {
                    ...
                    opts.dataLabels = { 
                        enabled: true, 
                        // Mencegah NaN jika nilainya null/0
                        formatter: function(val) { return (val || 0).toLocaleString('id-ID'); }, 
            ...
            renderComp() {
                ...
                let colors = this.tabComp === 'anggaran' ? ['#198754', '#fd7e14'] : ['#0d6efd', '#0dcaf0', '#20c997', '#ffc107'];
