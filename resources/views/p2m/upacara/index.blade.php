    ...
    <main class="admin-main">
        <div class="container-fluid p-4 p-lg-5">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h1 class="h3 mb-0">P2M Sosialisasi sebagai Pembina Upacara</h1>
                    <p class="text-muted mb-0">Master Data Sosialisasi sebagai Pembina Upacara</p>
                </div>
            </div>
                ...
                <div class="col-12 col-lg-12">
                    <div class="card shadow-lg p-5">
                        ...
                        <div class="card-body">
                            ...
                            <form action="{{ route('p2m.upacara.index') }}" method="GET" class="mb-8">
                                ...
                                <div class="row mb-5 align-items-center">
                                    <div class="col-auto">
                                        <div class="d-flex gap-2">
                                            ...
                                            {{-- @if ($activeFilters > 0)
                                                <a href="{{ route('p2m.upacara.index') }}" 
                                            ...
                                            <button type="submit" formaction="{{ route('p2m.upacara.export') }}"
                                ...
                                <table class="table table-hover mb-0" x-data="{ expanded: [] }">
                                    <thead class="table-light">
                                        <tr class="text-center align-middle">
                                            ...
                                            <th>
                                                <a href="{{ request()->fullUrlWithQuery(['sort_by' => 'satuan_kerja', 'sort_order' => request('sort_by') == 'satuan_kerja' && request('sort_order') == 'asc' ? 'desc' : 'asc']) }}"
                                                    ...
                                                    @if (request('sort_by') == 'satuan_kerja')
                                                        <i
                                                            class="bi bi-sort-{{ request('sort_order') == 'asc' ? 'alpha-down' : 'alpha-down-alt' }}"></i>
                                                    @else
                                            ...
                                            {{-- 3. NAMA SEKOLAH --}}
                                            <th>
                                                <a href="{{ request()->fullUrlWithQuery(['sort_by' => 'nama_sekolah', 'sort_order' => request('sort_by') == 'nama_sekolah' && request('sort_order') == 'asc' ? 'desc' : 'asc']) }}"
                                                    class="text-decoration-none text-dark d-flex justify-content-center align-items-center gap-1">
                                                    Nama Sekolah
                                                    @if (request('sort_by') == 'nama_sekolah')
                                                        <i
                                                            class="bi bi-sort-{{ request('sort_order') == 'asc' ? 'alpha-down' : 'alpha-down-alt' }}"></i>
                                                    @else
                                                        <i class="bi bi-arrow-down-up text-muted opacity-25 small"></i>
                                            ...
                                            <th>
                                                <a href="{{ request()->fullUrlWithQuery(['sort_by' => 'tanggal_pelaksanaan', 'sort_order' => request('sort_by') == 'tanggal_pelaksanaan' && request('sort_order') == 'asc' ? 'desc' : 'asc']) }}"
                                                    ...
                                                    @if (request('sort_by') == 'tanggal_pelaksanaan')
                                                        <i
                                                            class="bi bi-sort-numeric-{{ request('sort_order') == 'asc' ? 'down' : 'up-alt' }}"></i>
                                                    @else
                                                        <i class="bi bi-arrow-down-up text-muted opacity-25 small"></i>
                                                    @endif
                                                </a>
                                            ...
                                            {{-- 5. PEGAWAI (Tidak di-sort sesuai permintaan) --}}
                                            ...
                                            <th>
                                                <a href="{{ request()->fullUrlWithQuery(['sort_by' => 'created_at', 'sort_order' => request('sort_by') == 'created_at' && request('sort_order') == 'asc' ? 'desc' : 'asc']) }}"
                                                    ...
                                                    @if (!request('sort_by') || request('sort_by') == 'created_at')
                                                        {{-- Icon Aktif (Default) --}}
                                                        <i
                                                            class="bi bi-caret-{{ request('sort_order', 'desc') == 'asc' ? 'up' : 'down' }}-fill small ms-1"></i>
                                                    @else
                                                        <i
                                                            class="bi bi-arrow-down-up text-muted opacity-25 small ms-1"></i>
                                                    @endif
                                                </a>
                                            </th>
                                            <th>Aksi</th>
                                    ...
                                    <tbody>
                                        @forelse ($upacaras as $data)
                                            <tr class="text-center align-middle">
                                                <td>{{ $upacaras->firstItem() + $loop->index }}</td>
                                                <td>{{ $data->satuanKerja->satuan_kerja ?? '-' }}</td>
                                                <td>{{ $data->nama_sekolah }}</td>
                                                <td>{{ $data->tanggal_pelaksanaan->locale('id')->translatedFormat('l, d F Y') }}
                                                </td>
                                                <td class="text-start">
                                                        ...
                                                        <button type="button" class="btn btn-info btn-sm text-white"
                                                            ...
                                                            <i class="me-0 bi"
                                                                :class="expanded.includes({{ $data->id }}) ?
                                                                    'bi-eye-slash' : 'bi-eye'"></i>
                                                        </button>
                                                        <a href="{{ route('p2m.upacara.edit', $data->id) }}"
                                                            class="btn btn-success btn-sm"><i
                                                                class="me-0 bi bi-pencil-square"></i></a>
                                                        <form id="delete-form-{{ $data->id }}"
                                                            action="{{ route('p2m.upacara.destroy', $data->id) }}"
                                                            method="POST" class="d-inline">
                                                            @csrf @method('DELETE')
                                                            <button type="button" class="btn btn-danger btn-sm"
                                                                onclick="confirmDelete({{ $data->id }})"><i
                                                                    class="me-0 bi bi-trash"></i></button>
                                                        </form>
                                                    </div>
                                                </td>
                                            </tr>
                                            ...
                                            <tr x-show="expanded.includes({{ $data->id }})"
                                                x-transition.duration.200ms class="bg-light">
                                                <td colspan="10" class="text-start p-4">
                                                    <div class="card border-0">
                                                        <div class="card-body">
                                                            <h5 class="card-title fw-bold text-primary mb-3">Informasi
                                                                Tambahan</h5>
                                                            <div class="row">
                            ...
                            <div class="d-flex justify-content-between align-items-center mt-4">
                                ...
                                <div class="d-flex align-items-center gap-2">
                                    ...
                                    <select class="form-select form-select-sm border-secondary-subtle"
                                        style="width: auto;" onchange="window.location.href = this.value">
                                ...
                                <div>
                                    {{ $upacaras->fragment('data-table')->links() }}
                                </div>
                            </div>
                        </div>
                    </div>
