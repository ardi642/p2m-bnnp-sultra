    ...
    <main class="admin-main">
        <div class="container-fluid p-4 p-lg-5">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h1 class="h3 mb-1 fw-bold text-dark">Kegiatan P2M Desa Bersinar</h1>
                    <p class="text-muted mb-0">Master Data Pencanangan Desa Bersinar</p>
                </div>
            </div>
            ...
            @php
                ...
                $sortLink = function($col, $label) {
                    ...
                    return '<a href="'.request()->fullUrlWithQuery(['sort_by' => $col, 'sort_order' => $newOrder]).'" class="text-decoration-none text-secondary fw-bold d-flex align-items-center justify-content-between gap-2">'.$label.' <i class="bi '.$icon.'"></i></a>';
                };
            @endphp
                    ...
                    <div class="card border-0 shadow-sm"> 
                        <div class="card-header bg-white py-3 border-bottom">
                            <div class="d-flex flex-column flex-lg-row justify-content-between align-items-center gap-2">
                                <h5 class="card-title mb-0 fw-bold text-secondary"><i class="bi bi-table me-2"></i>Data Desa Bersinar</h5>
                                <button type="button" @click="showFilter = !showFilter" class="btn btn-sm transition-all d-flex align-items-center gap-2" :class="showFilter ? 'btn-light text-secondary border' : 'btn-primary shadow-sm'">
                                    <i class="bi" :class="showFilter ? 'bi-chevron-up' : 'bi-funnel'"></i> <span x-text="showFilter ? 'Sembunyikan Filter' : 'Filter Pencarian'"></span>
                        ...
                        <div class="card-body p-0 p-lg-4">
                            <form action="{{ route('p2m.desa-bersinar.index') }}" method="GET">
                                    ...
                                    <div class="bg-body-tertiary p-4 rounded-3 border">
                                        <div class="row g-3 text-start">
                                            ...
                                            <div class="col-12 text-end pt-3 border-top mt-4 text-start">
                                                <a href="{{ route('p2m.desa-bersinar.index') }}" class="btn btn-link text-decoration-none text-muted btn-sm me-2">Reset</a>
                                ...
                                <table class="table table-hover align-middle mb-0" x-data="{ expanded: [] }">
                                    <thead class="bg-light sticky-top">
                                        <tr class="text-center align-middle small text-uppercase text-secondary text-nowrap">
                                            ...
                                            <th class="py-3 bg-light text-start">{!! $sortLink('nama_desa', 'Desa') !!}</th>
                                            <th class="py-3 bg-light text-start">{!! $sortLink('nama_kelurahan', 'Kelurahan') !!}</th>
                                            ...
                                            <th class="py-3 bg-light text-start">PJ (Pegawai)</th>
                                    ...
                                    <tbody class="border-top-0">
                                        @forelse ($desas as $data)
                                            <tr class="text-center align-middle" :class="expanded.includes({{ $data->id }}) ? 'bg-light' : ''">
                                                ...
                                                <td class="text-start fw-bold text-dark"><a href="#" class="text-decoration-none fw-bold text-dark" @click.prevent="expanded.includes({{ $data->id }}) ? expanded = expanded.filter(id => id !== {{ $data->id }}) : expanded.push({{ $data->id }})">{{ $data->nama_desa }}</a></td>
                                                
                                                <td class="text-start">{{ $data->nama_kelurahan }}</td>
                                                ...
                                                <td class="small text-muted text-nowrap">{{ $data->created_at->locale('id')->translatedFormat('d M Y H:i') }}</td>
                                                <td class="pe-3">
                                                                ...
                                                                <div class="row g-4 text-start">
                                                                    <div class="col-lg-6">
                                                                        <dl class="row mb-0 small">
                                                                            <dt class="col-sm-4 text-secondary mb-2">Desa / Kelurahan</dt><dd class="col-sm-8 text-dark fw-bold">{{ $data->nama_desa }} / {{ $data->nama_kelurahan }}</dd>
...
<script type="module">
    document.addEventListener("DOMContentLoaded", function() {
        ...
        const ids = ['select-satker', 'select-bulan', 'select-anggaran', 'select-tahun', 'select-pegawai', 'select-kabkota'];
    ...
    window.confirmDelete = function(id) {
        Swal.fire({
            ...
            showCancelButton: true, confirmButtonColor: '#dc3545', cancelButtonColor: '#6c757d', confirmButtonText: 'Ya, Hapus', cancelButtonText: 'Batal', reverseButtons: true
