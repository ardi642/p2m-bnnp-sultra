...
use App\Models\P2mIkan;
use App\Models\SatuanKerja;
use App\Models\Pegawai;
use Illuminate\View\View;
use Illuminate\Http\Request;
use App\Exports\IkanExport;
...
class IkanController extends Controller
{
    ...
    private function getFilteredQuery(Request $request)
    {
        ...
        $query = P2mIkan::with('pegawai.satuanKerja', 'satuanKerja');
        ...
        if (in_array($sortBy, $allowSort)) {
            if ($sortBy === 'satuan_kerja') {
                $query->join('satuan_kerja', 'p2m_ikan.satuan_kerja_id', '=', 'satuan_kerja.id')
    ...
    public function index(Request $request): View
    {
        ...
        $yearQuery = P2mIkan::selectRaw('YEAR(tanggal_pelaksanaan) as year');
        ...
        $ikans = $query->paginate($perPage)->withQueryString();

        $satkerLookup = SatuanKerja::pluck('satuan_kerja', 'id')->toArray();

        return view('p2m.ikan.index', compact('ikans', 'satuanKerjas', 'years', 'pegawais', 'user', 'satkerLookup', 'totalKegiatan', 'totalPeserta'));
    }
    ...
    public function create(): View
    {
        ...
        return view('p2m.ikan.create', compact('satuanKerjas', 'pegawais'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            // 1. Simpan Kegiatan
            $kegiatan = P2mIkan::create($dataKegiatan);
        ...
        return redirect()->route('p2m.ikan.index')->with('success', 'store')->with('message', 'Berhasil menambahkan data.');
    }
    ...
    public function edit($id): View
    {
        /** @var \App\Models\User $user */
        $user = Auth::user();
        $kegiatan = P2mIkan::with('pegawai')->findOrFail($id);
        ...
        return view('p2m.ikan.edit', compact('kegiatan', 'satuanKerjas', 'pegawais', 'selectedPegawaiNips'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            // Sync Pegawai
            $oldPivotData = DB::table('pegawai_p2m_ikan')->where('p2m_ikan_id', $id)->get()->keyBy('pegawai_nip');
            ...
            return redirect()->route('p2m.ikan.index')->with('success', 'update')->with('message', 'Data berhasil diperbarui');
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    {
        $query = $this->getFilteredQuery($request);
        return Excel::download(new IkanExport($query), 'Laporan_P2M_Ikan.xlsx');
    }
