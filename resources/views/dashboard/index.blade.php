        ...
        <div x-show="activeTab === 'rehab'" x-transition>
            ...
            <div class="d-flex justify-content-end mb-3">
                <div class="d-flex align-items-center bg-white p-2 rounded shadow-sm border gap-2">
                    ...
                    <select x-model="startYear" class="form-select form-select-sm border-success fw-bold text-success" style="width: 90px;">@foreach($years as $y) <option value="{{ $y }}">{{ $y }}</option> @endforeach</select>
                    <span class="fw-bold">-</span>
                    <select x-model="endYear" class="form-select form-select-sm border-success fw-bold text-success" style="width: 90px;">@foreach($years as $y) <option value="{{ $y }}">{{ $y }}</option> @endforeach</select>
                </div>
            </div>
            ...
            <div class="row g-3 mb-4">
                {{-- KARTU 1 --}}
                <div class="col-md-4">
                    <div class="card border-0 shadow-sm h-100 p-4 text-white position-relative" style="background-color: #198754;">
                        <h6 class="text-uppercase fw-bold opacity-75 mb-3 small">RAWAT JALAN</h6>
                        <div class="d-flex justify-content-between align-items-end mb-3">
                            <div><small class="d-block opacity-75">Realisasi</small><h2 class="fw-bold m-0" x-text="rehabCards.rj.realisasi">0</h2></div>
                            <div class="text-end"><span class="display-6 fw-bold" x-text="rehabCards.rj.pct + '%'">0%</span><small class="d-block opacity-75">Capaian</small></div>
                        </div>
                        <div class="bg-white bg-opacity-10 p-2 rounded small">Target Tahunan: <span class="fw-bold" x-text="rehabCards.rj.target">0</span></div>
                    </div>
                </div>
                {{-- KARTU 2 --}}
                <div class="col-md-4">
                    <div class="card border-0 shadow-sm h-100 p-4 text-white position-relative" style="background-color: #20c997;">
                        <h6 class="text-uppercase fw-bold opacity-75 mb-3 small">PASCA REHABILITASI</h6>
                        <div class="d-flex justify-content-between align-items-end mb-3">
                            <div><small class="d-block opacity-75">Realisasi</small><h2 class="fw-bold m-0" x-text="rehabCards.pr.realisasi">0</h2></div>
                            <div class="text-end"><span class="display-6 fw-bold" x-text="rehabCards.pr.pct + '%'">0%</span><small class="d-block opacity-75">Capaian</small></div>
                        </div>
                        <div class="bg-white bg-opacity-10 p-2 rounded small">Target Tahunan: <span class="fw-bold" x-text="rehabCards.pr.target">0</span></div>
                    </div>
                </div>
                {{-- KARTU 3 --}}
                <div class="col-md-4">
                    <div class="card border-0 shadow-sm h-100 p-4 text-white position-relative" style="background-color: #0dcaf0;">
                        <h6 class="text-uppercase fw-bold opacity-75 mb-3 small">LAYANAN SKHPN</h6>
                        <div class="d-flex justify-content-between align-items-end mb-3">
                            <div><small class="d-block opacity-75">Realisasi</small><h2 class="fw-bold m-0" x-text="rehabCards.sk.realisasi">0</h2></div>
                            <div class="text-end"><span class="display-6 fw-bold" x-text="rehabCards.sk.pct + '%'">0%</span><small class="d-block opacity-75">Capaian</small></div>
                        </div>
                        <div class="bg-white bg-opacity-10 p-2 rounded small">Target Tahunan: <span class="fw-bold" x-text="rehabCards.sk.target">0</span></div>
                    </div>
                </div>
            </div>
            ...
            <div class="card border-0 shadow-sm p-4 mt-4">
                ...
                {{-- Manual Render agar x-ref terbaca jelas --}}
                <div class="mb-5 border-bottom pb-4">
                    <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-3">
                        <h6 class="fw-bold text-success mb-0 fs-5">1. LAYANAN RAWAT JALAN</h6>
                        <div class="d-flex gap-2">
                            <span class="badge bg-light text-dark border fs-6 p-2">Target: <span x-text="rehabTrendSummary.rj.t"></span></span>
                            <span class="badge bg-light text-dark border fs-6 p-2">Realisasi: <span x-text="rehabTrendSummary.rj.r"></span></span>
                            <span class="badge fs-6 p-2" :class="rehabTrendSummary.rj.p >= 80 ? 'bg-success text-white' : 'bg-warning text-dark'">Capaian: <span x-text="rehabTrendSummary.rj.p + '%'"></span></span>
                        </div>
                    </div>
                    <div x-ref="chartRehabrj" style="min-height: 400px;"></div>
                </div>

                <div class="mb-5 border-bottom pb-4">
                    <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-3">
                        <h6 class="fw-bold text-success mb-0 fs-5" style="color: #20c997 !important;">2. PASCA REHABILITASI</h6>
                        <div class="d-flex gap-2">
                            <span class="badge bg-light text-dark border fs-6 p-2">Target: <span x-text="rehabTrendSummary.pr.t"></span></span>
                            <span class="badge bg-light text-dark border fs-6 p-2">Realisasi: <span x-text="rehabTrendSummary.pr.r"></span></span>
                            <span class="badge fs-6 p-2" :class="rehabTrendSummary.pr.p >= 80 ? 'bg-success text-white' : 'bg-warning text-dark'">Capaian: <span x-text="rehabTrendSummary.pr.p + '%'"></span></span>
                        </div>
                    </div>
                    <div x-ref="chartRehabpr" style="min-height: 400px;"></div>
                </div>

                <div>
                    <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-3">
                        <h6 class="fw-bold text-info mb-0 fs-5">3. LAYANAN SKHPN</h6>
                        <div class="d-flex gap-2">
                            <span class="badge bg-light text-dark border fs-6 p-2">Target: <span x-text="rehabTrendSummary.sk.t"></span></span>
                            <span class="badge bg-light text-dark border fs-6 p-2">Realisasi: <span x-text="rehabTrendSummary.sk.r"></span></span>
                            <span class="badge fs-6 p-2" :class="rehabTrendSummary.sk.p >= 80 ? 'bg-success text-white' : 'bg-warning text-dark'">Capaian: <span x-text="rehabTrendSummary.sk.p + '%'"></span></span>
                        </div>
                    </div>
                    <div x-ref="chartRehabsk" style="min-height: 400px;"></div>
                </div>
            </div>
        </div>
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('dashboardUnified', () => ({
            ...
            // --- Chart Instances ---
            charts: {
                p2mRanking: null, left: null, right: null,
                chartLknFull: null, chartTatFull: null, chartBbFull: null,
                chartRehabrj: null, chartRehabpr: null, chartRehabsk: null
            },

            // =================================================================
            // 2. INISIALISASI & WATCHERS
            // =================================================================
            init() {
                // Setup TomSelect untuk Filter Satker
                ...
                // Watchers Global (Ganti Tab / Filter Utama)
            ...
            // =================================================================
            // 3. FUNGSI P2M (LOGIKA ASLI - HANYA FORMATTING)
            // =================================================================
            fetchP2MGlobal() {
            ...
            renderLeftChart(data) {
                ...
                if (data.config.has_positif) {
                    series.push({ name: 'Positif', type: 'line', data: data.tren.positif });
                }
            ...
            renderRightChart(data) {
                if (!data || !this.$refs.rightChart) return;
                
                let isAnggaran = this.rightChartMode === 'anggaran';
                let series = isAnggaran 
                    ? [{ name: 'DIPA', data: data.anggaran.dipa }, { name: 'Non-DIPA', data: data.anggaran.non_dipa }] 
                    : data.sasaran;
            ...
            // =================================================================
            // 4. FUNGSI BERANTAS (LOGIKA ASLI - HANYA FORMATTING)
            // =================================================================
            fetchBerantasGlobal() {
            ...
            fetchBerantasChart() {
                ...
                fetch(url).then(res => res.json()).then(data => {
                    ...
                    // LKN Chart (Blue Theme + Black Line)
                    ...
                    // TAT Chart (Cyan Theme + Black Line)
                    ...
                    // Register Chart (Red Theme + Black Line)
            ...
            renderRehabGrouped(el, data, color, chartKey) {
                ...
                let options = {
                    ...
                    chart: { type: 'bar', height: 400, toolbar: { show: true }, fontFamily: 'Nunito' },
                    plotOptions: {
                        bar: {
                            horizontal: false,
                            columnWidth: '60%',
                    ...
                    colors: ['#e9ecef', color], // Abu-abu (Target), Warna (Realisasi)
                    
                    // --- BAGIAN INI YANG DIUBAH UNTUK MENAMPILKAN PERSEN ---
                    dataLabels: {
                        enabled: true,
                        offsetY: -20, // Geser label sedikit ke atas
                        style: {
                            fontSize: '11px',
                            fontWeight: 'bold',
                            colors: ['#333']
                        ...
                        formatter: function (val, opts) {
                            // Jika nilai 0, sembunyikan biar bersih
                            if (val === 0) return "";

                            // Cek apakah ini batang "Realisasi" (Series Index 1)
                            if (opts.seriesIndex === 1) {
                                // Ambil nilai Target pasangannya di bulan yang sama
                                let targetVal = data.t[opts.dataPointIndex];

                                // Hitung Persentase: (Realisasi / Target) * 100
                                if (targetVal > 0) {
                                    let pct = Math.round((val / targetVal) * 100);
                                    // Tampilkan format: "Nilai (Persen%)" -> Contoh: "20 (167%)"
                                    return val + " (" + pct + "%)";
                                }
                            }
                            
                            // Untuk batang Target, tampilkan angkanya saja
                            return val;
                        }
                    },
                    // -------------------------------------------------------
                    ...
                    yaxis: {
                        title: { text: 'Jumlah', style: { fontWeight: 'bold' } }
                    },
                    tooltip: {
                        shared: true,
                        intersect: false,
                        y: {
                            formatter: function (val, opts) {
                                // Tambahkan juga info persen di tooltip agar lebih detail saat disentuh
                                if (opts.seriesIndex === 1) {
                                    let targetVal = data.t[opts.dataPointIndex];
                                    if (targetVal > 0) {
                                        let pct = Math.round((val / targetVal) * 100);
                                        return val + " (" + pct + "% dari Target)";
                                    }
                                }
                                return val;
                            }
            ...
            get satkerLabel() {
                ...
                return el && el.options[el.selectedIndex] 
