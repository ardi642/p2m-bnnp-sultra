...
{
    Schema::create('pegawai_p2m_sosialisasi', function (Blueprint $table) {
        $table->id();

        // 1. Relasi ke Kegiatan (Masih pakai ID/Integer standar)
        $table->foreignId('p2m_sosialisasi_id')
            ->constrained('p2m_sosialisasi')
            ->onDelete('cascade');
    ...
    {
        Schema::dropIfExists('pegawai_p2m_sosialisasi');
    }
};
