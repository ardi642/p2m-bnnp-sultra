            ...
            <div class="row justify-content-center mb-4">
                <div class="col-12 col-lg-10">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <h1 class="h3 mb-1 fw-bold text-dark">Edit Lingkungan Bersinar</h1>
                            <p class="text-muted mb-0">Perbarui Data Kawasan/Wilayah Bersih Narkoba</p>
                        </div>
                        <a href="{{ route('p2m.lingkungan-bersinar.index') }}" class="btn btn-outline-secondary d-flex align-items-center gap-2">
            ...
            <div class="row justify-content-center">
                <div class="col-12 col-lg-10">
                    <div class="card border-0 shadow-lg">
                        <div class="card-header bg-white py-3 border-bottom"><h5 class="card-title mb-0 fw-bold">Form Edit Data</h5></div>
                        <div class="card-body p-4 p-lg-5">
                            
                            <form action="{{ route('p2m.lingkungan-bersinar.update', $data->id) }}" method="POST" enctype="multipart/form-data" id="form-edit">
                                @csrf 
                                @method('PUT')
                                ...
                                <div class="row g-4 mb-5">
                                    ...
                                    <div class="col-12 col-lg-6">
                                        ...
                                        <select class="form-select" name="satuan_kerja_id">
                                            @foreach ($satuanKerjas as $s) 
                                                <option value="{{ $s->id }}" @selected(old('satuan_kerja_id', $data->satuan_kerja_id) == $s->id)>{{ $s->satuan_kerja }}</option> 
                                            @endforeach
                                        </select>
                                    </div>
                                    @endif
                                    ...
                                    <div class="col-12 col-lg-{{ auth()->user()->isAdmin() ? '6' : '12' }}">
                                        ...
                                        <select class="form-select" name="sasaran_kegiatan">
                                            <option value="sekolah/kampus bersinar" @selected(old('sasaran_kegiatan', $data->sasaran_kegiatan) == 'sekolah/kampus bersinar')>Lingkungan Kerja</option>
                                            <option value="pondok pesantren bersinar" @selected(old('sasaran_kegiatan', $data->sasaran_kegiatan) == 'pondok pesantren bersinar')>Pondok Pesantren Bersinar</option>
                                            <option value="tempat hiburan bersinar" @selected(old('sasaran_kegiatan', $data->sasaran_kegiatan) == 'tempat hiburan bersinar')>Tempat Hiburan Bersinar</option>
                                            <option value="tempat wisata bersinar" @selected(old('sasaran_kegiatan', $data->sasaran_kegiatan) == 'tempat wisata bersinar')>Tempat Wisata Bersinar</option>
                                            <option value="industri bersinar" @selected(old('sasaran_kegiatan', $data->sasaran_kegiatan) == 'industri bersinar')>Industri Bersinar</option>
                                        </select>
                                    </div>
                                    
                                    <div class="col-12">
                                        <label class="form-label fw-semibold text-secondary small">Nama Tempat / Wilayah / Instansi <span class="text-danger">*</span></label>
                                        <textarea class="form-control" name="nama_tempat_wilayah" rows="3" placeholder="Masukkan nama tempat, wilayah, atau instansi lengkap">{{ old('nama_tempat_wilayah', $data->nama_tempat_wilayah) }}</textarea>
                                    </div>
                                </div>
                                ...
                                <div class="row g-4 mb-5">
                                    <div class="col-12 col-md-6">
                                        ...
                                        <input type="date" class="form-control" name="tanggal_pencanangan" value="{{ old('tanggal_pencanangan', $data->tanggal_pencanangan->format('Y-m-d')) }}">
                                    </div>
                                    
                                    <div class="col-12 col-md-6">
                                        <label class="form-label fw-semibold text-secondary small">Jumlah Penggiat P4GN <span class="text-danger">*</span></label>
                                        <div class="input-group">
                                            <input type="number" class="form-control" name="jumlah_penggiat_p4gn" value="{{ old('jumlah_penggiat_p4gn', $data->jumlah_penggiat_p4gn) }}" placeholder="Masukkan jumlah">
                                    ...
                                    <div class="col-12 col-md-6">
                                        ...
                                        <input type="text" class="form-control" name="no_hp_penanggung_jawab" value="{{ old('no_hp_penanggung_jawab', $data->no_hp_penanggung_jawab) }}" placeholder="Masukkan nomor HP">
                                    </div>
                                    ...
                                    <div class="col-12 col-md-6">
                                        ...
                                        <select id="select-pegawai" name="pegawai_nips[]" multiple placeholder="Pilih pegawai...">
                                            @foreach ($pegawais as $pgw)
                                                @php $selected = collect(old('pegawai_nips', $data->pegawai->pluck('nip')->toArray()))->contains($pgw->nip); @endphp
                                                <option value="{{ $pgw->nip }}" @selected($selected)>{{ $pgw->nama }} ({{ $pgw->nip }})</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>

                                {{-- SECTION 3: DOKUMENTASI (TAMPILAN DIPERBARUI) --}}
                                ...
                                {{-- BUTTONS --}}
                                <div class="d-flex flex-column-reverse flex-lg-row justify-content-end gap-2 pt-3 border-top mt-5">
...
@push('styles')
    ...
    <style>
        ...
        /* Style baru untuk border tebal merah saat ditandai hapus */
...
<script type="module">
    ...
    // 4. Logic Hapus File Lama (Diperbarui Class-nya)
