<aside class="admin-sidebar bg-white" x-data x-init="// FITUR PENTING: Mengembalikan posisi scroll dari memori browser
    ...
    <div class="sidebar-content py-3">
        ...
        ['route' => 'p2m.pemetaan-sdm-sda.index', 'label' => 'Pemetaan SDM dan SDA', 'url' => 'p2m/pemetaan-sdm-sda*'],
            ...
            @if (auth()->user()->hasRole(['admin', 'admin_satker', 'admin_berantas', 'operator_satker', 'operator_berantas']))
                ...
                @foreach ([['route' => 'berantas.narkotika.index', 'label' => 'Data Narkotika', 'url' => 'berantas/narkotika*'], ['route' => 'berantas.tat.index', 'label' => 'Tim Asesmen (TAT)', 'url' => 'berantas/tat*'], ['route' => 'berantas.ungkap-kasus.index', 'label' => 'Ungkap Kasus', 'url' => 'berantas/ungkap-kasus*'], ['route' => 'berantas.peta-ungkap-kasus.index', 'label' => 'Pemetaan Ungkap Kasus', 'url' => 'berantas/peta-ungkap-kasus*'], ['route' => 'berantas.register-barang-bukti.index', 'label' => 'Barang Bukti', 'url' => 'berantas/register-barang-bukti*'], ['route' => 'berantas.peta-sebaran-bb.index', 'label' => 'Pemetaan Barang Bukti', 'url' => 'berantas/peta-sebaran-bb*']] as $menu)
                    <a href="{{ route($menu['route']) }}"
                        class="nav-link {{ Request::is($menu['url']) ? 'active' : '' }}">
                        {{ $menu['label'] }}
                    </a>
            ...
            @if (auth()->user()->hasRole(['admin', 'admin_satker', 'admin_rehab', 'operator_satker', 'operator_rehab']))
                ...
                @foreach ([['route' => 'rehab.laporan.index', 'label' => 'Laporan Tahunan', 'url' => 'rehab/laporan*'], ['route' => 'rehab.pasien.index', 'label' => 'Daftar Pasien', 'url' => 'rehab/pasien*']] as $menu)
                    <a href="{{ route($menu['route']) }}"
                        class="nav-link {{ Request::is($menu['url']) ? 'active' : '' }}">
                        {{ $menu['label'] }}
                    </a>
