                    ...
                    <div class="card border-0 shadow-sm"> 
                        <div class="card-header bg-white py-3 border-bottom">
                            <div class="d-flex flex-column flex-lg-row justify-content-between align-items-center gap-2">
                                <h5 class="card-title mb-0 fw-bold text-secondary"><i class="bi bi-moisture me-2"></i>Data Tes Urine</h5>
                        ...
                        <div class="card-body p-0 p-lg-4">
                            ...
                            {{-- FORM FILTER --}}
                            <form action="{{ route('p2m.tes-urine.index') }}" method="GET">
                                        ...
                                        <div class="row g-3 text-start">
                                            ...
                                            <div class="{{ $user->isAdmin() ? 'col-lg-8' : 'col-12' }}">
                                                ...
                                                <div class="input-group shadow-sm">
                                                    ...
                                                    <input type="text" name="search" class="form-control border-start-0 ps-0" placeholder="Cari Instansi, Tempat, Parameter, dll..." value="{{ request('search') }}">
                                                </div>
                                            </div>
                                            ...
                                            <div class="col-12 col-lg-6">
                                                <label class="form-label fw-bold small text-secondary text-uppercase mb-1 d-block">Panitia / Pegawai</label>
                                            ...
                                            <div class="col-12 text-end pt-3 border-top mt-4 text-start">
                                                <a href="{{ route('p2m.tes-urine.index') }}" class="btn btn-link text-decoration-none text-muted btn-sm me-2">Reset</a>
                                ...
                                <div class="d-flex flex-column flex-lg-row justify-content-between align-items-end align-items-lg-center mb-3 px-3 px-lg-0">
                                    ...
                                    <div class="mb-2 mb-lg-0">
                                        <button type="submit" formaction="{{ route('p2m.tes-urine.export') }}" class="btn btn-success btn-sm text-white d-flex align-items-center gap-2 px-3 shadow-none">
                                    ...
                                    <div class="d-flex gap-2">
                                        ...
                                        <div class="d-flex align-items-center border border-secondary-subtle rounded-3 px-3 py-1 bg-light">
                                            ...
                                            <span class="text-muted" style="font-size: 0.9rem;">Total Peserta Tes Urine : </span>
                                            <span class="text-dark ms-1" style="font-size: 0.9rem;">
                                        ...
                                        <div class="d-flex align-items-center border border-secondary-subtle rounded-3 px-3 py-1 bg-light">
                                            <i class="bi bi-person-exclamation text-muted me-2" style="font-size: 0.85rem;"></i>
                                            <span class="text-muted" style="font-size: 0.9rem;">Total Peserta  Terindikasi Positif : </span>
                                            <span class="text-dark ms-1" style="font-size: 0.9rem;">
                                                {{ number_format($totalPositif, 0, ',', '.') }}
                                            </span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <table class="table table-hover align-middle mb-0" x-data="{ expanded: [] }">
                                    <thead class="bg-light sticky-top">
                                        <tr class="text-center align-middle small text-uppercase text-secondary text-nowrap">
                                            ...
                                            <th class="py-3 bg-light text-start">{!! $sortLink('nama_instansi', 'Instansi Pelaksana') !!}</th>
                                            ...
                                            <th class="py-3 bg-light text-start">Panitia</th>
                                            <th class="py-3 bg-light">{!! $sortLink('jumlah_peserta', 'Peserta') !!}</th>
                                            <th class="py-3 bg-light">{!! $sortLink('jumlah_positif', 'Positif') !!}</th>
                                    ...
                                    <tbody class="border-top-0">
                                        @forelse ($kegiatans as $data)
                                            <tr class="text-center align-middle" :class="expanded.includes({{ $data->id }}) ? 'bg-light' : ''">
                                                ...
                                                <td class="text-start"><a href="#" class="text-decoration-none fw-bold text-dark" @click.prevent="expanded.includes({{ $data->id }}) ? expanded = expanded.filter(id => id !== {{ $data->id }}) : expanded.push({{ $data->id }})">{{ $data->nama_instansi }}</a></td>
                                                <td>
                                                    @php 
                                                ...
                                                <td>
                                                    @if($data->jumlah_positif > 0)
                                                        <span class="badge bg-danger text-white">{{ $data->jumlah_positif }}</span>
                                                    @else
                                                ...
                                                <td class="pe-3">
                                                    <div class="btn-group btn-group-sm shadow-sm">
                                                        ...
                                                        @if (auth()->user()->hasRole(['operator_satker', 'operator_p2m']))
                                                            <a href="{{ route('p2m.tes-urine.edit', $data->id) }}" class="btn btn-light border text-primary" title="Edit"><i class="bi bi-pencil-square"></i></a>
                                                            ...
                                                            <form id="delete-form-{{ $data->id }}" action="{{ route('p2m.tes-urine.destroy', $data->id) }}" method="POST" class="d-none">@csrf @method('DELETE')</form>
                                                        @endif
                                                    </div>
                                                </td>
                                            </tr>
                                                                ...
                                                                <div class="row g-4 text-start">
                                                                    <div class="col-lg-6">
                                                                        <dl class="row mb-0 small text-start">
                                                                            <dt class="col-sm-4 text-secondary mb-2">Instansi Pelaksana</dt>
                                                                            <dd class="col-sm-8 text-dark">{{ $data->nama_instansi }}</dd>
                                                                            ...
                                                                            <dt class="col-sm-4 text-secondary mb-2">Tempat / Alamat</dt>
                                                                        ...
                                                                        <div class="p-3 bg-light rounded mt-3 border">
                                                                            <h6 class="small fw-bold text-secondary text-uppercase border-bottom pb-2 mb-2">Hasil Tes Urine</h6>
                                                                            <div class="d-flex justify-content-between mb-2 small">
                                                                                <span>Total Peserta:</span>
                                                                                <span class="fw-bold">{{ $data->jumlah_peserta }} Orang</span>
                                                                            </div>
                                                                            <div class="d-flex justify-content-between mb-2 small">
                                                                                <span>Terindikasi Positif:</span>
                                                                                <span class="fw-bold text-danger">{{ $data->jumlah_positif }} Orang</span>
                                                                            </div>
                                                                            <div class="small">
                                                                                <span class="d-block text-secondary mb-1">Keterangan Parameter:</span>
                                                                                <div class="p-2 bg-white border rounded text-dark fst-italic">
                                                                                    {{ $data->keterangan_positif ?? 'Tidak ada keterangan' }}
                                                                                </div>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                    <div class="col-lg-6">
                                                                        ...
                                                                        <div class="mb-4 text-start">
                                                                            <span class="fw-bold text-secondary small d-block mb-2 text-start">Panitia Pelaksana:</span>
                                        ...
                                        @empty
                                            <tr><td colspan="11" class="text-center py-5 text-muted fst-italic border-bottom">Tidak ada data kegiatan Tes Urine.</td></tr>
                                        @endforelse
                                    </tbody>
                                </table>
                            </div>
