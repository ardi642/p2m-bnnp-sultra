                    ...
                    <div class="card shadow-lg p-5">
                        <div class="card-header">
                            <div class="row align-items-center">
                                <div class="col">
                                    <h5 class="card-title mb-0 text-center">Data Tes Urine</h5>
                                </div>
                            </div>
                        </div>
                        <div class="card-body">
                                ...
                                <table class="table table-hover mb-0" x-data="{ expanded: [] }">
                                    <thead class="table-light">
                                        <tr class="text-center">
                                            ...
                                            <th>Nama Instansi Pelaksana</th>
                                            <th>Tanggal Pelaksanaan</th>
                                            <th>Jumlah Peserta</th>
                                            <th>Jumlah Terindikasi Positif</th>
                                            <th>Aksi</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @forelse ($tesUrins as $data)
                                            {{-- Baris Utama --}}
                                            <tr class="text-center align-middle">
                                                <td>{{ $tesUrins->firstItem() + $loop->index }}</td>
                                                <td>{{ $data->satuanKerja->satuan_kerja ?? '-' }}</td>
                                                <td>{{ $data->sasaran_kegiatan ?? '-' }}</td>
                                                <td>{{ $data->nama_instansi_pelaksana ?? '-' }}</td>
                                                <td>
                                                    {{ \Carbon\Carbon::parse(time: $data->tanggal_pencanangan)->translatedFormat('d F Y') }}
                                                </td>
                                                <td>{{ $data->jumlah_peserta_test_urin }}</td>
                                                <td>{{ $data->jumlah_terindikasi_positif }}</td>
                                            ...
                                            <tr>
                                                <td colspan="10" class="text-center p-4">
                                                    <div class="text-muted">
                                                        Belum ada data kegiatan tes urine
                                                    </div>
                                                </td>
                                            </tr>
                                        @endforelse
