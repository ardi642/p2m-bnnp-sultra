...
@section('content')
@php
    $allFilters = request()->only(['search', 'bulan', 'tahun', 'satuan_kerja_id', 'jenis_kelamin', 'pendidikan', 'pekerjaan', 'sumber_pasien', 'narkotika_ids']);
        ...
        <div class="card border-0 shadow-sm mb-4">
            ...
            <div class="card-body p-0 p-lg-4">
                        ...
                        <div class="row g-3">
                            <div class="col-12">
                                ...
                                <div class="input-group shadow-sm">
                                    ...
                                    <input type="text" name="search" class="form-control border-start-0 ps-0" placeholder="Cari No Rekam Medis / Nama Pasien..." value="{{ request('search') }}">
                                </div>
                            </div>
                            ...
                            <div class="col-lg-4">
                                ...
                                <select id="sel-satker" name="satuan_kerja_id[]" multiple placeholder="Pilih Satuan Kerja...">
                                    @foreach($satuanKerjas as $sk) <option value="{{ $sk->id }}" {{ in_array($sk->id, request('satuan_kerja_id', [])) ? 'selected' : '' }}>{{ $sk->satuan_kerja }}</option> @endforeach
                                </select>
                            </div>
                            @endif
                            ...
                            <div class="{{ Auth::user()->isAdmin() ? 'col-lg-4' : 'col-md-6' }}">
                                ...
                                <select id="sel-tahun" name="tahun[]" multiple placeholder="Pilih Tahun...">
                                    @foreach($years as $y) <option value="{{ $y }}" {{ in_array($y, request('tahun', [date('Y')])) ? 'selected' : '' }}>{{ $y }}</option> @endforeach
                                </select>
                            </div>
                            <div class="{{ Auth::user()->isAdmin() ? 'col-lg-4' : 'col-md-6' }}">
                            ...
                            <div class="col-md-3">
                                ...
                                <select id="sel-sumber" name="sumber_pasien[]" multiple placeholder="Pilih Sumber...">
                                    <option value="Voluntary" {{ in_array('Voluntary', request('sumber_pasien', [])) ? 'selected' : '' }}>Voluntary</option>
                                    <option value="Compulsory" {{ in_array('Compulsory', request('sumber_pasien', [])) ? 'selected' : '' }}>Compulsory</option>
                                </select>
                            </div>
                            <div class="col-md-3">
                                ...
                                <select id="sel-kerja" name="pekerjaan[]" multiple placeholder="Pilih Pekerjaan...">
                                    @foreach(\App\Constants\Pekerjaan::ALL as $p) <option value="{{ $p }}" {{ in_array($p, request('pekerjaan', [])) ? 'selected' : '' }}>{{ $p }}</option> @endforeach
                                </select>
                            </div>
                            <div class="col-md-3">
                                ...
                                <select id="sel-didik" name="pendidikan[]" multiple placeholder="Pilih Pendidikan...">
                                    @foreach(\App\Constants\Pendidikan::ALL as $p) <option value="{{ $p }}" {{ in_array($p, request('pendidikan', [])) ? 'selected' : '' }}>{{ $p }}</option> @endforeach
                                </select>
                            </div>
                            ...
                            <div class="col-12 mt-3">
                                ...
                                <select id="sel-narko" name="narkotika_ids[]" multiple placeholder="Cari Narkotika...">
                                    @foreach($masterNarkotika as $n) <option value="{{ $n->id }}" {{ in_array($n->id, request('narkotika_ids', [])) ? 'selected' : '' }}>{{ $n->nama_narkotika }}</option> @endforeach
                                </select>
                            </div>
                ...
                <div class="d-flex justify-content-between mb-3 px-3 px-lg-0">
                    ...
                    <div class="text-muted small border border-secondary-subtle rounded px-3 py-1 bg-light">Total Riwayat Kedatangan: <strong>{{ $data->total() }}</strong></div>
                </div>
                    ...
                    <table class="table table-hover align-middle mb-0 text-nowrap">
                        <thead class="bg-light sticky-top">
                            <tr class="small text-uppercase text-secondary">
                                ...
                                <th class="py-3 bg-light">{!! $sortLink('no_rekam_medis', 'No. Rekam Medis') !!}</th>
                                <th class="py-3 bg-light">{!! $sortLink('nama_pasien', 'Nama Pasien') !!}</th>
                                <th class="py-3 bg-light">{!! $sortLink('jenis_kelamin', 'Jenis Kelamin') !!}</th>
                                <th class="py-3 bg-light">{!! $sortLink('tanggal_rehab', 'Tgl Kedatangan') !!}</th>
                                <th class="py-3 bg-light text-center">{!! $sortLink('usia', 'Usia') !!}</th>
                        ...
                        <tbody class="border-top-0">
                            ...
                            <tr class="align-middle">
                                ...
                                <td class="py-3"><span class="badge bg-primary-subtle text-primary border border-primary-subtle">{{ $row->pasien->satuanKerja->satuan_kerja ?? '-' }}</span></td>
                                <td class="py-3 fw-bold font-monospace text-dark">{{ $row->pasien->no_rekam_medis }}</td>
                                ...
                                <td class="py-3 text-center">{{ $row->usia }} Thn</td>
                                <td class="py-3">{{ $row->pekerjaan }}</td>
                                <td class="py-3">{{ $row->pendidikan }}</td>
                                <td class="py-3 text-center"><span class="badge {{ $row->sumber_pasien == 'Voluntary' ? 'bg-success-subtle text-success' : 'bg-danger-subtle text-danger' }} border">{{ $row->sumber_pasien }}</span></td>
                                <td class="py-3 text-start">
                                    <div class="text-wrap" style="max-width: 150px;">
                                        @foreach($row->narkotika as $n) <span class="badge bg-secondary mb-1">{{ $n->nama_narkotika }}</span> @endforeach
                                    </div>
                                </td>
                                <td class="py-3 text-center small text-muted">{{ $row->created_at->format('d/m/Y H:i') }}</td>
                                <td class="py-3 pe-3 text-center">
                                    <div class="btn-group btn-group-sm shadow-sm">
                                        <a href="{{ route('rehab.pasien.show', $row->pasien->id) }}" class="btn btn-light border border-secondary-subtle" title="Lihat Rekam Medis"><i class="bi bi-eye text-primary"></i></a>
                                        @if (auth()->user()->hasRole(['operator_satker', 'operator_rehab', 'admin']))
                                            <a href="{{ route('rehab.pasien.riwayat.create', $row->pasien->id) }}" class="btn btn-light border border-secondary-subtle" title="Tambah Riwayat Baru"><i class="bi bi-plus-circle text-success"></i></a>
                                            <a href="{{ route('rehab.pasien.riwayat.edit', ['id' => $row->id, 'ref' => 'index']) }}" class="btn btn-light border border-secondary-subtle" title="Edit Riwayat Ini"><i class="bi bi-pencil-square text-warning"></i></a>
                                            <button type="button" class="btn btn-light border border-secondary-subtle text-danger" onclick="confirmDelete({{ $row->id }})" title="Hapus Riwayat"><i class="bi bi-trash"></i></button>
                                            <form id="delete-form-{{ $row->id }}" action="{{ route('rehab.pasien.destroy', $row->id) }}" method="POST" class="d-none">@csrf @method('DELETE')</form>
                                        @endif
                                    </div>
                                </td>
                            </tr>
                            @empty
                            <tr><td colspan="13" class="text-center py-5 text-muted fst-italic">Tidak ada data pasien ditemukan.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
...
<script>
    ...
    window.confirmDelete = function(id) {
        Swal.fire({
            title: 'Hapus Riwayat?', text: "Data kedatangan ini akan dihapus permanen.", icon: 'warning',
