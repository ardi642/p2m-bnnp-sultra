...
class RtsController extends Controller
{
    ...
    private function getFilteredQuery(Request $request)
    {
        ...
        $query = P2mRts::with('pegawai.satuanKerja', 'satuanKerja');
        ...
        if (in_array($sortBy, $allowSort)) {
            if ($sortBy === 'satuan_kerja') {
                $query->join('satuan_kerja', 'p2m_rts.satuan_kerja_id', '=', 'satuan_kerja.id')
    ...
    public function index(Request $request): View 
    {
        ...
        $yearQuery = P2mRts::selectRaw('YEAR(tanggal_pelaksanaan) as year');
        ...
        $rtss = $query->paginate($perPage)->withQueryString();

        $satkerLookup = SatuanKerja::pluck('satuan_kerja', 'id')->toArray();
                        
        return view('p2m.rts.index', compact('rtss', 'satuanKerjas', 'years', 'pegawais', 'user', 'satkerLookup', 'totalKegiatan', 'totalPeserta'));
    }
    ...
    public function create(): View 
    {
        ...
        return view('p2m.rts.create', compact('satuanKerjas', 'pegawais'));
    }
    ...
    public function store(Request $request, DokumenService $dokumenService) {
        ...
        try {
            ...
            // 1. Simpan Kegiatan
            $kegiatan = P2mRts::create($dataKegiatan);
            ...
            DB::commit();
            return redirect()->route('p2m.rts.index')->with('success', 'store')->with('message', 'Berhasil menambahkan data.');
    ...
    public function edit($id): View 
    {
        /** @var \App\Models\User $user */
        $user = Auth::user();
        $kegiatan = P2mRts::with('pegawai')->findOrFail($id);
        ...
        return view('p2m.rts.edit', compact('kegiatan', 'satuanKerjas', 'pegawais', 'selectedPegawaiNips'));
    }
    ...
    public function update(Request $request, DokumenService $dokumenService, $id) 
    {
        ...
        try {
            ...
            // Sync Pegawai
            $oldPivotData = DB::table('pegawai_p2m_rts')->where('p2m_rts_id', $id)->get()->keyBy('pegawai_nip');
            ...
            return redirect()->route('p2m.rts.index')->with('success', 'update')->with('message', 'Data diperbarui');
    ...
    {
        $query = $this->getFilteredQuery($request);
        return Excel::download(new RtsExport($query), 'Laporan_P2M_RTS.xlsx');
    }
