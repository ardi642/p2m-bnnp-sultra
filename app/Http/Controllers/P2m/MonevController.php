...
use App\Models\P2mMonev; // Model Baru
use App\Models\SatuanKerja;
use App\Models\Pegawai;
use Illuminate\View\View;
use Illuminate\Http\Request;
use App\Exports\MonevExport; // Export Baru
...
class MonevController extends Controller
{
    private function getFilteredQuery(Request $request)
    {
        ...
        // Model diganti
        $query = P2mMonev::with('pegawai.satuanKerja', 'satuanKerja');
        ...
        if (in_array($sortBy, $allowSort)) {
            if ($sortBy === 'satuan_kerja') {
                // Join tabel baru
                $query->join('satuan_kerja', 'p2m_monev.satuan_kerja_id', '=', 'satuan_kerja.id')
    ...
    public function index(Request $request): View 
    {
        ...
        $yearQuery = P2mMonev::selectRaw('YEAR(tanggal_pelaksanaan) as year');
        ...
        // View folder baru
        return view('p2m.monev.index', compact('kegiatans', 'satuanKerjas', 'years', 'pegawais', 'user', 'satkerLookup', 'totalKegiatan', 'totalPeserta'));
    }
    ...
    public function create(): View 
    {
        ...
        // View folder baru
        return view('p2m.monev.create', compact('satuanKerjas', 'pegawais'));
    }
    ...
    public function store(Request $request, DokumenService $dokumenService) {
        ...
        try {
            ...
            $kegiatan = P2mMonev::create($dataKegiatan);
            ...
            return redirect()->route('p2m.monev.index')->with('success', 'store')->with('message', 'Berhasil menambahkan data.');
    ...
    public function edit($id): View 
    {
        ...
        $user = Auth::user();
        // Model Baru
        $kegiatan = P2mMonev::with('pegawai')->findOrFail($id);
        ...
        // View Baru
        return view('p2m.monev.edit', compact('kegiatan', 'satuanKerjas', 'pegawais', 'selectedPegawaiNips'));
    }
    ...
    public function update(Request $request, DokumenService $dokumenService, $id) 
    {
        /** @var \App\Models\User $user */
        $user = Auth::user();
        $kegiatan = P2mMonev::findOrFail($id);
        ...
        try {
            ...
            $oldPivotData = DB::table('pegawai_p2m_monev')->where('p2m_monev_id', $id)->get()->keyBy('pegawai_nip');
            ...
            return redirect()->route('p2m.monev.index')->with('success', 'update')->with('message', 'Data diperbarui');
    ...
    {
        $query = $this->getFilteredQuery($request);
        return Excel::download(new MonevExport($query), 'Laporan_P2M_Monev.xlsx');
    }
}