            ...
            <div class="card shadow-sm border-0 mb-4">
                ...
                <div class="card-body p-4">
                    <div class="row g-3">
                        ...
                        {{-- MAPS & KOORDINAT --}}
                        {{-- FIX: Menggunakan x-data terpisah dan init --}}
                        <div class="col-12" x-data="locationPicker">
                            ...
                            <div class="row g-2 mb-2">
                                ...
                                <div class="col-2">
                                    {{-- Tombol GPS dengan Indikator Loading --}}
                                    <button type="button" 
                                        ...
                                        {{-- TAMPILAN SAAT LOADING (MUNCUL JIKA isLoading = true) --}}
                                        <span x-show="isLoading" style="display: none;"> {{-- style display none agar tidak kedip saat load awal --}}
                                            <span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>
                                            <span class="d-none d-md-inline ms-1">...</span>
                                        </span>

                                        {{-- TAMPILAN NORMAL (MUNCUL JIKA isLoading = false) --}}
                            ...
                            {{-- FIX: Wire:ignore agar tidak dirender ulang oleh Livewire/Alpine sembarangan --}}
                        ...
                        <div class="col-12">
                            ...
                            <textarea name="kronologis" class="form-control auto-resize @error('kronologis') is-invalid @enderror" rows="3" placeholder="Ceritakan kronologis singkat penangkapan..." x-data="{ resize() { $el.style.height = 'auto'; $el.style.height = $el.scrollHeight + 'px' } }" x-init="resize()" @input="resize()">{{ old('kronologis') }}</textarea>
            ...
            <div class="card shadow-sm border-0 mb-4">
                <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
                    ...
                    <button type="button" class="btn btn-dark btn-sm px-3" @click="addMaster">
                        <i class="bi bi-person-plus-fill me-1"></i> Tambah Tersangka
                    </button>
                </div>
                            ...
                            <tbody>
                                <template x-for="(tsk, idx) in masterSuspects" :key="tsk.temp_id">
                                    <tr>
                                        ...
                                        <td class="text-center p-2 bg-white align-top">
                                            ...
                                            <template x-if="hasError('tersangka', idx, 'foto')">
                                                <div class="text-danger small mt-1" style="font-size: 0.65rem;" x-text="getErrorMessage('tersangka', idx, 'foto')"></div>
                                            </template>
                                        </td>
...
<script type="module">
    ...
    document.addEventListener('alpine:init', () => {
        ...
        // --- 1. FIX LOGIKA MAPS / GPS (create.blade.php) ---
        Alpine.data('locationPicker', () => ({
            // Gunakan logika manual agar aman dari syntax error jika old kosong
            lat: {{ old('latitude') ? old('latitude') : 'null' }},
            lng: {{ old('longitude') ? old('longitude') : 'null' }},
            map: null, 
            marker: null,
            isLoading: false, // Variable wajib untuk loading tombol
            ...
            getGPS() { 
                // 1. Aktifkan status loading (Tombol akan berputar)
                this.isLoading = true;
                ...
                if(navigator.geolocation) {
                    navigator.geolocation.getCurrentPosition(
                        ...
                        { 
                            ...
                            timeout: 10000,           // TUNGGU SAMPAI 10 DETIK
            ...
            init() {
                ...
                if(oldTsk.length > 0) {
                      oldTsk.forEach(t => this.masterSuspects.push({ 
                          ...
                          nama: t.nama || '', jk: t.jk || 'Laki-Laki', pekerjaan: t.pekerjaan || '', tahap: t.tahap || '', preview: null 
                      }));
                } else {
                      this.addMaster();
                }
                if(oldGroups.length > 0) {
                    oldGroups.forEach(g => {
                        this.bbGroups.push({
                            id: 'g_'+Math.random(),
                            selectedOwners: g.owners || [],
                            items: (g.items || []).map(i => ({
                                id: Math.random(),
                                kategori: i.kategori || 'Narkotika',
                                narkotika_id: i.narkotika_id ? i.narkotika_id : '', 
                                nama_barang_bukti: i.nama_barang_bukti || '',
                                jumlah: i.jumlah || '',
                                satuan: i.satuan || 'Gram'
                            }))
                        });
                    });
                } else {
