...
use App\Models\P2mInformasiEdukasi; // Model Baru
use App\Models\SatuanKerja;
use App\Models\Pegawai;
use Illuminate\View\View;
use Illuminate\Http\Request;
use App\Exports\InformasiEdukasiExport; // Export Baru
...
class InformasiEdukasiController extends Controller
{
    private function getFilteredQuery(Request $request)
    {
        ...
        // Model diganti
        $query = P2mInformasiEdukasi::with('pegawai.satuanKerja', 'satuanKerja');
        ...
        if (in_array($sortBy, $allowSort)) {
            if ($sortBy === 'satuan_kerja') {
                // Join tabel baru
                $query->join('satuan_kerja', 'p2m_informasi_edukasi.satuan_kerja_id', '=', 'satuan_kerja.id')
                        ->orderBy('satuan_kerja.satuan_kerja', $sortOrder)
                        ->select('p2m_informasi_edukasi.*');
            } else {
    ...
    public function index(Request $request): View 
    {
        ...
        $yearQuery = P2mInformasiEdukasi::selectRaw('YEAR(tanggal_pelaksanaan) as year');
        ...
        // View folder baru
        return view('p2m.informasi-edukasi.index', compact('kegiatans', 'satuanKerjas', 'years', 'pegawais', 'user', 'satkerLookup', 'totalKegiatan', 'totalPeserta'));
    }
    ...
    public function create(): View 
    {
        ...
        // View folder baru
        return view('p2m.informasi-edukasi.create', compact('satuanKerjas', 'pegawais'));
    }
    ...
    public function store(Request $request, DokumenService $dokumenService) {
        ...
        try {
            ...
            $kegiatan = P2mInformasiEdukasi::create($dataKegiatan);
            ...
            return redirect()->route('p2m.informasi-edukasi.index')->with('success', 'store')->with('message', 'Berhasil menambahkan data.');
    ...
    public function edit($id): View 
    {
        ...
        $user = Auth::user();
        // Model Baru
        $kegiatan = P2mInformasiEdukasi::with('pegawai')->findOrFail($id);
        ...
        // View Baru
        return view('p2m.informasi-edukasi.edit', compact('kegiatan', 'satuanKerjas', 'pegawais', 'selectedPegawaiNips'));
    }
    ...
    public function update(Request $request, DokumenService $dokumenService, $id) 
    {
        /** @var \App\Models\User $user */
        $user = Auth::user();
        $kegiatan = P2mInformasiEdukasi::findOrFail($id);
        ...
        try {
            ...
            $oldPivotData = DB::table('pegawai_p2m_informasi_edukasi')->where('p2m_informasi_edukasi_id', $id)->get()->keyBy('pegawai_nip');
            ...
            return redirect()->route('p2m.informasi-edukasi.index')->with('success', 'update')->with('message', 'Data diperbarui');
    ...
    {
        $query = $this->getFilteredQuery($request);
        return Excel::download(new InformasiEdukasiExport($query), 'Laporan_P2M_Informasi_Edukasi.xlsx');
    }
}