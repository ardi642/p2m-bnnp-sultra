...
<main class="admin-main" x-data="{ showFilter: true, expanded: [] }">
    <div class="container-fluid p-4 p-lg-5">
        ...
        @php
            $allFilters = request()->only(['search', 'bulan', 'tahun', 'satuan_kerja_id', 'kategori_bb', 'sumber_perolehan', 'narkotika_ids', 'search_non_narkotika']);
                ...
                <div class="card border-0 shadow-sm"> 
                    <div class="card-header bg-white py-3 border-bottom">
                        <div class="d-flex flex-column flex-lg-row justify-content-between align-items-center gap-2">
                            <h5 class="card-title mb-0 fw-bold text-secondary"><i class="bi bi-table me-2"></i>Data Register Barang Bukti</h5>
                    ...
                    <div class="card-body p-0 p-lg-4">
                        ...
                        {{-- FORM FILTER --}}
                        <form action="{{ route('berantas.register-barang-bukti.index') }}" method="GET">
                            ...
                            <div x-show="showFilter" x-transition class="mb-4 px-3 px-lg-0 pt-3 pt-lg-0" x-data="fileDownloader">
                                <div class="bg-body-tertiary p-4 rounded-3 border">
                                    <div class="row g-3 text-start">
                                        ...
                                        {{-- Row 3: Sumber, Bulan, Tahun --}}
                                        <div class="col-lg-4">
                                            <label class="form-label fw-bold small text-secondary text-uppercase mb-1">Sumber Perolehan</label>
                                            <div class="shadow-sm bg-white rounded">
                                                <select id="select-sumber" name="sumber_perolehan[]" multiple placeholder="Pilih Sumber..." autocomplete="off">
                                                    <option value="Hasil Tangkap" {{ in_array('Hasil Tangkap', request('sumber_perolehan', [])) ? 'selected' : '' }}>Hasil Tangkap</option>
                                                    <option value="Temuan" {{ in_array('Temuan', request('sumber_perolehan', [])) ? 'selected' : '' }}>Temuan</option>
                                                </select>
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-12 text-end pt-3 border-top mt-4 text-start">
                                            <a href="{{ route('berantas.register-barang-bukti.index') }}" class="btn btn-link text-decoration-none text-muted btn-sm me-2">Reset Filter</a>
                                ...
                                <tbody class="border-top-0">
                                    @forelse ($data as $row)
                                        <tr class="align-top" :class="expanded.includes({{ $row->id }}) ? 'bg-light' : ''">
                                            ...
                                            <td class="text-start py-3">
                                                <span class="small text-muted text-nowrap">{{ $row->tanggal_perolehan->locale('id')->translatedFormat('d M Y') }}</span>
                                            </td>
                                                    ...
                                                    <div class="d-flex flex-column gap-1">
                                                        @foreach($row->items as $item)
                                                            <div class="small d-flex align-items-center">
                                                                ...
                                                                @endif
                                                                <span class="text-dark me-1 fw-semibold">{{ $item->nama_barang }}</span>
                                                                <span class="text-muted">({{ (float)$item->kuantitas }} {{ $item->satuan }})</span>
                                                            </div>
                                                        @endforeach
                                                    </div>
                                                @else
...
<script type="module">
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        ['select-satker', 'select-bulan', 'select-tahun', 'select-sumber', 'select-narkotika'].forEach(id => { 
            if(document.getElementById(id)) new TomSelect('#' + id, configTomSelect); 
        });

        // Kategori BB Listener
        if(document.getElementById('select-kategori-bb')) {
            ...
            function updateBBVisibility() {
                ...
                if (selected.length > 0) {
                    if(selected.includes('Narkotika') && wrapperNarkotika) {
                        wrapperNarkotika.style.display = 'block';
                    }
                    if(selected.includes('Non-Narkotika') && wrapperNonNarkotika) {
                        wrapperNonNarkotika.style.display = 'block';
                    }
                }
            }
            // Jalankan saat load
        ...
        // Non Narkotika Text Input (Tagging)
    ...
    window.confirmDelete = function(id) {
        Swal.fire({
            title: 'Hapus Data?', text: "Data register ini akan dihapus permanen.", icon: 'warning',
