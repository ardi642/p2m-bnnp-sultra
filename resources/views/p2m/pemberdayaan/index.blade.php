        ...
        <div class="container-fluid p-4 p-lg-5">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    ...
                    <p class="text-muted mb-0">Master Data Pemberdayaan Alternatif</p>
                </div>
            ...
            @php
                $allFilters = request()->only(['satuan_kerja_id', 'bulan', 'tahun', 'anggaran_pelaksanaan', 'sasaran_kegiatan', 'sub_kegiatan', 'detail_kegiatan', 'search', 'pegawai_nips']);
                    ...
                    <div class="card border-0 shadow-sm">
                        <div class="card-header bg-white py-3 border-bottom">
                            <div class="d-flex flex-column flex-lg-row justify-content-between align-items-center gap-2">
                                <h5 class="card-title mb-0 fw-bold text-secondary"><i class="bi bi-table me-2"></i>Data Pemberdayaan Alternatif</h5>
                        ...
                        <div class="card-body p-0 p-lg-4">
                            ...
                            <form action="{{ route('p2m.pemberdayaan.index') }}" method="GET">
                                ...
                                {{-- FILTER BUNGKUS DENGAN ALPINE JS 'filterPemberdayaan' --}}
                                <div x-show="showFilter" x-transition class="mb-4 px-3 px-lg-0 pt-3 pt-lg-0" x-data="filterPemberdayaan">
                                    <div class="bg-body-tertiary p-4 rounded-3 border">
                                        <div class="row g-3 text-start">
                                            
                                            <div class="{{ $user->isAdmin() ? 'col-lg-6' : 'col-12' }}">
                                            ...
                                            {{-- Sub Kegiatan (Alpine TomSelect) --}}
                                            <div class="col-lg-4">
                                                <label class="form-label fw-bold small text-secondary text-uppercase mb-1">Sub Kegiatan</label>
                                                <div class="shadow-sm bg-white rounded">
                                                    <select x-ref="sub" name="sub_kegiatan[]" multiple placeholder="Pilih Sub Kegiatan..." autocomplete="off">
                                                        @foreach($subKegiatanList as $val => $label)
                                                            <option value="{{ $val }}" {{ in_array($val, request('sub_kegiatan', [])) ? 'selected' : '' }}>{{ $label }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            </div>

                                            {{-- Detail Kegiatan (Alpine TomSelect Dependent) --}}
                                            <div class="col-lg-4">
                                                <label class="form-label fw-bold small text-secondary text-uppercase mb-1">Detail Kegiatan</label>
                                                <div class="shadow-sm bg-white rounded">
                                                    @php $reqDetail = request('detail_kegiatan', []); @endphp
                                                    <select x-ref="detail" name="detail_kegiatan[]" multiple placeholder="Pilih Detail Kegiatan..." autocomplete="off">
                                                        @foreach($allDetailKegiatan as $val => $label)
                                                            <option value="{{ $val }}" {{ in_array($val, $reqDetail) ? 'selected' : '' }}>{{ $label }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            </div>
                                            ...
                                            <div class="col-12 text-end pt-3 border-top mt-4 text-start">
                                                <a href="{{ route('p2m.pemberdayaan.index') }}" class="btn btn-link text-decoration-none text-muted btn-sm me-2">Reset</a>
                                ...
                                <div class="d-flex flex-column flex-lg-row justify-content-between align-items-end align-items-lg-center mb-3 px-3 px-lg-0">
                                    <div class="mb-2 mb-lg-0">
                                        <button type="submit" formaction="{{ route('p2m.pemberdayaan.export') }}" class="btn btn-success btn-sm text-white d-flex align-items-center gap-2 px-3 shadow-none">
                            ...
                            <div class="custom-table-scroll mb-3" id="data-table">
                                <table class="table table-hover align-middle mb-0">
                                    ...
                                    <tbody class="border-top-0">
                                            ...
                                            <tr class="text-center align-middle" :class="isExpanded({{ $data->id }}) ? 'bg-light' : ''">
                                                ...
                                                <td class="pe-3">
                                                    <div class="btn-group btn-group-sm shadow-sm">
                                                        ...
                                                        @if (auth()->user()->hasRole(['operator_satker', 'operator_p2m']))
                                                            <a href="{{ route('p2m.pemberdayaan.edit', $data->id) }}" class="btn btn-light border text-primary" title="Edit"><i class="bi bi-pencil-square"></i></a>
                                                            ...
                                                            <form id="delete-form-{{ $data->id }}" action="{{ route('p2m.pemberdayaan.destroy', $data->id) }}" method="POST" class="d-none">@csrf @method('DELETE')</form>
                                                        @endif
                                                    </div>
                                                </td>
                                            </tr>
                                                                ...
                                                                <div class="row g-4 text-start mb-4">
                                                                    <div class="col-lg-6">
                                                                        <dl class="row mb-0 small text-start">
                                                                            ...
                                                                            <dt class="col-sm-4 text-secondary mb-2">Sub Kegiatan</dt>
                                                                            <dd class="col-sm-8 text-dark text-capitalize">{{ $data->sub_kegiatan_label }}</dd>
                                                                            <dt class="col-sm-4 text-secondary mb-2">Detail Kegiatan</dt>
                                                                            <dd class="col-sm-8 text-dark">{{ $data->detail_kegiatan_label }}</dd>
                                                                            ...
                                                                            <ul class="list-unstyled mb-0 small text-start">
                                                                                @forelse ($allPegawai as $pegawai)
                                                                                    ...
                                                                                    <li class="mb-2 text-start">
                                                                                        ...
                                                                                        <span class="text-dark">{{ $pegawai->nama }}</span>
                                        ...
                                        @empty
                                            <tr>
                                                <td colspan="11" class="text-center py-5 text-muted fst-italic border-bottom">Tidak ada data kegiatan.</td>
                                            </tr>
                                        @endforelse
                                    </tbody>
                                </table>
...
@push('scripts')
    <script type="module">
        document.addEventListener('alpine:init', () => {
            // 1. Data Global 'p2mIndex' (Filter & Expand Table)
            ...
            // 2. Data Lokal 'fileDownloader' (Checkbox Logic)
            ...
            // 3. Data 'filterPemberdayaan' (Dependent Dropdown untuk TomSelect di Filter)
            Alpine.data('filterPemberdayaan', () => ({
                map: @json($detailKegiatanMap),
                allDetails: @json($allDetailKegiatan),
                tsSub: null,
                tsDetail: null,

                init() {
                    this.tsDetail = new TomSelect(this.$refs.detail, {
                    ...
                    this.tsSub = new TomSelect(this.$refs.sub, {
                        plugins: ['remove_button', 'clear_button'],
                        create: false,
                        maxOptions: null,
                        onChange: (values) => this.updateDetailOptions(values)
                    });

                    // Set opsi awal berdasarkan sub kegiatan yang sudah terpilih saat page dimuat
                    this.updateDetailOptions(this.tsSub.getValue(), true);
                },

                updateDetailOptions(selectedSubs, isInit = false) {
                    let validOptions = [];

                    // Jika tidak ada sub kegiatan terpilih, tampilkan semua detail
                    if (!selectedSubs || selectedSubs.length === 0) {
                        for (const [val, lbl] of Object.entries(this.allDetails)) {
                            validOptions.push({id: val, title: lbl});
                        }
                    } else {
                        // Kumpulkan detail berdasarkan sub yang terpilih
                        selectedSubs.forEach(sub => {
                            if (this.map[sub]) {
                                for (const [val, lbl] of Object.entries(this.map[sub])) {
                                    validOptions.push({id: val, title: lbl});
                                }
                            }
                        });
                    ...
                    let currentSelected = this.tsDetail.getValue();
                    this.tsDetail.clearOptions();

                    validOptions.forEach(opt => {
                        this.tsDetail.addOption(opt);
                    });

                    // Set kembali nilai yang dipilih jika masih valid
                    if (!isInit) {
                        let validIds = validOptions.map(o => o.id);
                        let newSelected = (currentSelected || []).filter(val => validIds.includes(val));
                        this.tsDetail.setValue(newSelected);
                    } else {
                        this.tsDetail.setValue(currentSelected);
                    }
                }
            }));
        });
