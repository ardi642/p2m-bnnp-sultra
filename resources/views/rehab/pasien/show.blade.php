                                    ...
                                    <tr :class="expanded.includes({{ $r->id }}) ? 'bg-light' : ''">
                                        ...
                                        <td class="text-center">
                                            <div class="btn-group btn-group-sm">
                                                <button type="button" class="btn btn-light border" @click="expanded.includes({{ $r->id }}) ? expanded = expanded.filter(id => id !== {{ $r->id }}) : expanded.push({{ $r->id }})" title="Lihat Dokumen"><i class="bi" :class="expanded.includes({{ $r->id }}) ? 'bi-chevron-up text-primary' : 'bi-folder2-open text-primary'"></i></button>
                                                @if (auth()->user()->hasRole(['operator_satker', 'operator_rehab', 'admin']))
                                                    <a href="{{ route('rehab.pasien.riwayat.edit', ['id' => $r->id, 'ref' => 'show']) }}" class="btn btn-light border text-warning" title="Edit Riwayat Kedatangan"><i class="bi bi-pencil-square"></i></a>
                                                @endif
                                            </div>
                                        </td>
                                    </tr>
                                        ...
                                        <td colspan="8" class="p-0 border-0">
                                            <div class="bg-body-tertiary p-4 border-bottom shadow-inner text-start">
                                                <div class="card border-0 shadow-sm">
                                                    <div class="card-body">
                                                        <h6 class="fw-bold text-primary border-bottom pb-2 mb-3"><i class="bi bi-paperclip me-2"></i>Dokumentasi & Lampiran</h6>
                                                                            ...
                                                                            <div class="card-body p-2" style="max-height: 250px; overflow-y: auto;">
                                                                                @forelse($fotos as $doc)
                                                                                    <div class="d-flex align-items-center bg-white border rounded p-2 mb-2 shadow-sm" :class="isSelected({{ $doc->id }}) ? 'border-primary bg-primary bg-opacity-10' : ''">
                                                                                        @if(!$doc->is_link)<div class="form-check me-2 d-flex align-items-center"><input class="form-check-input shadow-none cursor-pointer" type="checkbox" name="ids[]" value="{{ $doc->id }}" x-model="selected"></div>@endif
...
<script type="module">
    document.addEventListener('alpine:init', () => {
        Alpine.data('fileDownloader', () => ({
            ...
            toggle(id) { const strId = id.toString(); if (this.selected.includes(strId)) { this.selected = this.selected.filter(i => i !== strId); } else { this.selected.push(strId); } },
            toggleAll(ids) { const stringIds = ids.map(String); const allSelected = stringIds.every(id => this.selected.includes(id)); if (allSelected) { this.selected = this.selected.filter(id => !stringIds.includes(id)); } else { this.selected = [...new Set([...this.selected, ...stringIds])]; } },
            isAllSelected(ids) { if (ids.length === 0) return false; const stringIds = ids.map(String); return stringIds.every(id => this.selected.includes(id)); },
            submitDownload() { if (this.selected.length === 0) { Swal.fire({icon: 'warning', title: 'Pilih File', text: 'Silakan centang minimal satu file.'}); return; } this.$refs.formZip.submit(); }
        }));
    });
</script>
@endpush