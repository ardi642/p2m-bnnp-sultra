                                                ...
                                                <div class="shadow-sm bg-white rounded">
                                                    <select id="select-tahun" name="tahun[]" multiple
                                                        ...
                                                        @foreach ($tahuns as $tahun)
                                                            <option value="{{ $tahun }}"
                                                                {{ in_array($tahun, request('tahun', request()->has('tahun') ? [] : [$tahunSekarang])) ? 'selected' : '' }}>
                                                                {{ $tahun }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                                    ...
                                    <tbody class="border-top-0">
                                        @forelse ($pasien as $data)
                                            <tr class="text-center align-middle">
                                                ...
                                                <td><span class="fw-bold">{{ $data->usia }} tahun</span></td>
                                                <td class="text-center"><span class="fw-semibold text-dark">{{ $data->pekerjaan }}</span></td>
                                                <td>
                                                    <span class="fw-semibold text-dark">{{ $data->pendidikan }}</span></td>
                                                <td>
                                                    @if ($data->narkotika)
                                                        <span
                                                            class="badge rounded-pill bg-warning bg-opacity-10 text-warning border border-warning border-opacity-25">
                                                            {{ $data->narkotika->nama_narkotika }}
                                                        </span>
                                                    @else
                                                        <span class="text-muted">-</span>
                                                    @endif
                                            ...
                                            <tr x-show="isExpanded({{ $data->id }})" x-transition>
                                                <td colspan="11" class="p-0 border-0">
                                                    <div class="bg-body-tertiary p-4 border-bottom shadow-inner text-start">
                                                        <div class="card border-0 shadow-sm">
                                                            <div class="card-body">
                                                                <h6 class="card-title fw-bold text-primary border-bottom pb-2 mb-3"><i class="bi bi-info-circle me-2"></i>Detail Lengkap Pasien</h6>
                                                                
                                                                <div class="row g-4 text-start">
                                                                    <div class="col-lg-6">
                                                                        <dl class="row mb-0 small text-start">
                                                                            <dt class="col-sm-4 text-secondary mb-2">Nama Pasien</dt><dd class="col-sm-8 text-dark">{{ $data->nama_pasien }}</dd>
                                                                            <dt class="col-sm-4 text-secondary mb-2">Satuan Kerja</dt><dd class="col-sm-8 text-dark">{{ $data->satuanKerja->satuan_kerja ?? '-' }}</dd>
                                                                            <dt class="col-sm-4 text-secondary mb-2">Jenis Kelamin</dt><dd class="col-sm-8">
                                                                                <span class="badge rounded-pill {{ $data->jenis_kelamin == 'Laki-laki' ? 'bg-primary' : 'bg-danger' }} bg-opacity-10 {{ $data->jenis_kelamin == 'Laki-laki' ? 'text-primary' : 'text-danger' }} border {{ $data->jenis_kelamin == 'Laki-laki' ? 'border-primary' : 'border-danger' }} border-opacity-25">{{ $data->jenis_kelamin }}</span>
                                                                            </dd>
                                                                            <dt class="col-sm-4 text-secondary mb-2">Usia</dt><dd class="col-sm-8 text-dark">{{ $data->usia }} tahun</dd>
                                                                            <dt class="col-sm-4 text-secondary mb-2">Pekerjaan</dt><dd class="col-sm-8 text-dark">{{ $data->pekerjaan }}</dd>
                                                                            <dt class="col-sm-4 text-secondary mb-2">Pendidikan</dt><dd class="col-sm-8 text-dark">{{ $data->pendidikan }}</dd>
                                                                        </dl>
                                                                    </div>
                                                                    <div class="col-lg-6">
                                                                        <dl class="row mb-0 small text-start">
                                                                            <dt class="col-sm-4 text-secondary mb-2">Jenis Narkotika</dt><dd class="col-sm-8">
                                                                                @if ($data->narkotika)
                                                                                    <span class="badge rounded-pill bg-warning bg-opacity-10 text-warning border border-warning border-opacity-25">{{ $data->narkotika->nama_narkotika }}</span>
                                                                                @else
                                                                                    <span class="text-muted">-</span>
                                                                                @endif
                                                                            </dd>
                                                                            <dt class="col-sm-4 text-secondary mb-2">Sumber Pasien</dt><dd class="col-sm-8">
                                                                                <span class="badge rounded-pill bg-secondary bg-opacity-10 text-secondary border border-secondary border-opacity-25">{{ $data->sumber_pasien }}</span>
                                                                            </dd>
                                                                            <dt class="col-sm-4 text-secondary mb-2">Dibuat Pada</dt><dd class="col-sm-8 text-dark">{{ $data->created_at->locale('id')->translatedFormat('l, d F Y H:i') }}</dd>
                                                                            <dt class="col-sm-4 text-secondary mb-2">Terakhir Diubah</dt><dd class="col-sm-8 text-dark">{{ $data->updated_at->locale('id')->translatedFormat('l, d F Y H:i') }}</dd>
                                                                        </dl>
                                                                    </div>
                                                                </div>
                                                            </div>
